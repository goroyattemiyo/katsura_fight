...
## 2026-03-13 / v1.1.5 — カツラ吹き飛ばし演出改善

### 変更
- ゲームオーバー時、パーティクルではなくスタック上のカツラ自体が物理演算で弾け飛ぶ
- 各カツラに速度・重力・回転・フェードアウトを付与
- 上に積まれているカツラほど高く飛ぶ（i * 2 の初速加算）
- 追加でキラキラパーティクルも同時発生
- GAMEOVER_ANIM中はスタック描画を通常の固定描画から物理座標描画に切り替え
